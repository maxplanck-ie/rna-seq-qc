...
@
FASTQ: number of sequencing reads, TrimGalore: number of reads after trimming, HISAT2/TopHat2: number of mapped reads defined by samtools flags (SE: -F260 [primary alignment]; PE: -f3 -F260 [properly paired, min. quality 20, primary alignment]), featureCounts/htseq-count: counts of reads mapped to genes, \%: percentage compared to the starting number of sequencing reads
