...
if ( file.exists( indir ) ){
  ...
  for (file in files) {
    ...
    if (paired==T) {
      num = as.numeric( system(sprintf("%s view -c -f3 -q 20 %s | awk '{print $1/2}'", file.path(samtools_dir,"samtools"), file), intern=T) )
    } else {
      num = as.numeric( system(sprintf("%s view -c -q 20 %s", file.path(samtools_dir,"samtools"), file), intern=T) )
    }
    num_reads = c( num_reads, num )
    cat(paste(name, num, "\n"), sep=" ")
  }
